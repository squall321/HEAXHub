"""Tests for admin launcher-fleet observability + rotate-token:
GET /admin/agents (device_kind filter), /{id}, /{id}/installs, /{id}/audit,
...
def _seed(session: Session, agent_id) -> None:
    now = datetime.now(timezone.utc)
    session.add_all([
        InstallReport(agent_id=agent_id, app_id="koo-tool", version="1.0.0",
                      status="success", started_at=now, finished_at=now, sha256_verified=True),
        InstallReport(agent_id=agent_id, app_id="koo-tool", version="1.1.0",
...
def test_list_filters_by_device_kind(ctx) -> None:
    ...
    assert "obs-launcher-1" in names and "obs-service-1" not in names


def test_requires_admin_403(ctx) -> None:
    session, client = ctx
    user = _user(session, role=UserRole.USER, email="obs-user-1@example.com")
    assert client.get("/api/v1/admin/agents", headers=_admin(user)).status_code == 403


def test_detail_and_history(ctx) -> None:
    session, client = ctx
    admin = _user(session, role=UserRole.ADMIN, email="obs-admin-2@example.com")
    agent = _launcher(session, "obs-launcher-2")
    _seed(session, agent.id)
    aid = str(agent.id)

    d = client.get(f"/api/v1/admin/agents/{aid}", headers=_admin(admin))
    assert d.status_code == 200 and d.json()["device_kind"] == "launcher"

    ins = client.get(f"/api/v1/admin/agents/{aid}/installs", headers=_admin(admin))
    assert ins.status_code == 200
    assert {r["status"] for r in ins.json()} == {"success", "failed"}

    failed = client.get(f"/api/v1/admin/agents/{aid}/installs", params={"status": "failed"}, headers=_admin(admin))
    ...
    assert aud.json()[0]["action"] == "agent.install"
    assert aud.json()[0]["meta"]["actor"] == "system:hwax-agent"
...
def test_unknown_agent_404(ctx) -> None:
    ...
    m = str(uuid.uuid4())
    assert client.get(f"/api/v1/admin/agents/{m}", headers=_admin(admin)).status_code == 404
    assert client.get(f"/api/v1/admin/agents/{m}/installs", headers=_admin(admin)).status_code == 404
    assert client.get(f"/api/v1/admin/agents/{m}/audit", headers=_admin(admin)).status_code == 404
...
def test_rotate_token_mints_usable_token(ctx) -> None:
    ...
    resp = client.post(f"/api/v1/admin/agents/{agent.id}/rotate-token", headers=_admin(admin))
    assert resp.status_code == 200, resp.text
    token = resp.json()["token"]
    assert token
    enr = client.post("/api/v1/launcher-agents/enroll", json={"enrollment_token": token})
    assert enr.status_code == 200, enr.text  # the fresh token actually enrolls
...
def test_rotate_token_re_enables_disabled(ctx) -> None:
    ...
    resp = client.post(f"/api/v1/admin/agents/{agent.id}/rotate-token", headers=_admin(admin))
    assert resp.status_code == 200 and resp.json()["agent"]["disabled"] is False


def test_rotate_token_unknown_404(ctx) -> None:
    session, client = ctx
    admin = _user(session, role=UserRole.ADMIN, email="obs-rotate-3@example.com")
    assert client.post(f"/api/v1/admin/agents/{uuid.uuid4()}/rotate-token", headers=_admin(admin)).status_code == 404
