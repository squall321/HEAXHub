...
HWAXAgent 저장소는 이 폴더를 **git submodule** (`contracts/hwax-agent → HEAXHub/contracts/hwax-agent`) 로 끌어 씁니다. CI 빌드 시 `pnpm fetch` 단계에서 submodule SHA를 고정하고, C# DTO 생성기(NJsonSchema)가 `*.schema.json` 을 읽어 `Heax.Agent.Api` 어셈블리를 만듭니다.
